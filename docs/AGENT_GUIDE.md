...
| **Language (EN / pt-BR UI)** | [LANGUAGE.md](./LANGUAGE.md) |
| Architecture | [ARCHITECTURE.md](./ARCHITECTURE.md) |
| Schema | [DATABASE.md](./DATABASE.md) |
| Tasks | [BACKLOG.md](./BACKLOG.md) |
| Git flow & CI | [docs/AGENT_GUIDE.md](./AGENT_GUIDE.md) (Git flow section) |
...
| **Auth** | Full ASP.NET Identity + JWT on API. Roles in `AspNetRoles` / `AspNetUserRoles`. |
...
| **Email v1** | Resend `mail@onlineportfolio.com.br` — send only, no mailbox; Reply-To for replies. [ADR-017](./ADR-017-resend-transactional-email.md). No Google Workspace in v1. |
...
### CI: separate workflows (decision)

| | |
|---|---|
| **Chosen** | `ci-backend.yml` + `ci-frontend.yml` + `deploy-backend.yml` + `deploy-frontend.yml` |
| **Do not use** | Single `ci.yml` that always runs backend and frontend |
| **Monorepo** | One Git repo; separate workflows ≠ separate repos |

Details: [docs/ARCHITECTURE.md](./ARCHITECTURE.md) · [docs/ARCHITECTURE.md](./ARCHITECTURE.md) · [docs/EXTERNAL_PROVIDERS.md](./EXTERNAL_PROVIDERS.md)

**Environments:** cloud deploy **production only** (`main`). Dev = local Docker; PR = Vercel preview. No deployed staging in v1.

### PR CI (implemented — DEV-006)

Triggers on `pull_request` and `push` → `main`, with **path filters** — only runs the workflow for the stack that changed.

| Workflow | Status check | Commands | Paths (examples) |
|---|---|---|---|
| `ci-backend.yml` | **Backend CI** | `dotnet test` (Release) + Coverlet | `backend/**`, `docs/DATABASE.md`, deploy-backend workflow |
| `deploy-backend.yml` | **Backend Deploy** | `dotnet test` → `dotnet ef database update` (prod) | `push` → `main`; paths `backend/**`, `docs/DATABASE.md` |
| `ci-frontend.yml` | **Frontend CI** | `npm run lint` (typecheck) + `npm run test:coverage` (Vitest) | `frontend/**`, deploy-frontend workflow, coverage comment action |

**Path filters:** PR that changes only `docs/` or only `scripts/` may **not** trigger any CI — ok. PR that changes `backend/` **and** `frontend/` triggers **both**. Checks **skipped** by path filter count as OK on GitHub (branch protection).

**Deploy** (`deploy-backend.yml` ✅, `deploy-frontend.yml` ✅). Manual redeploy: **Run workflow** on deploy pipelines — [docs/DEV_COMMANDS.md](./DEV_COMMANDS.md).

### PR coverage comments

Only on **`pull_request`** (not on direct push to `main`). Publishes only if the workflow **ran** — no CI, no comment.

| Sticky comment | Header | Stack |
|---|---|---|
| **Backend coverage** | `backend-coverage` | Coverlet + `irongut/CodeCoverageSummary` |
| **Frontend coverage** | `frontend-coverage` | Vitest istanbul → inline script in `ci-frontend.yml` |

Two independent comments on the same PR when both CIs run. Artifacts (`TestResults/`, `frontend/coverage/`) stay in `.gitignore`.

### Pre-merge checklist

1. **Green CI** — `Backend CI` and/or `Frontend CI` per paths changed in PR.
2. **Coverage** — review sticky comments if present (informational; no % gate in v1).
3. **Paired components** — if API contract, schema, auth, env, or surface changed, update **both sides** (or document exception in PR description):
...
| Admin feature | Protected routes | UI in `components/app/`, host `app.*`, dark mode (`surface-dark`) |
| Public tenant site | Public API (published content only) | `{slug}.*`, `public/tenants/{slug}/`, `themes/{slug}.css` ([docs/ARCHITECTURE.md](./ARCHITECTURE.md)) |
| Email / Storage (future phases) | Resend / multipart API | Proxy or UI if applicable |

4. **Docs** — `DATABASE.md` if schema; `frontend/.env.example` or `appsettings` if new env vars; `EXTERNAL_PROVIDERS.md` if new provider secret.
5. **Commits** — [Conventional Commits](./CONVENTIONAL_COMMITS.md); `backend` / `frontend` scope consistent with touched paths.

**Agents:** on every PR, verify the pairing table even when the diff touches only one side — ask: "does the other side need follow-up **in this** PR?".

**Branch protection (recommended on `main`):** require **Backend CI** + **Frontend CI** (skipped checks = OK).
...
- **Language:** **English** for code, comments, docs, commits, API identifiers; **pt-BR only** for product UI and user-facing API/email copy — [LANGUAGE.md](./LANGUAGE.md).
...
- **Frontend:** thin pages, three surfaces — **admin = Nuxt UI, minimal motion**; **tenant `{slug}.*` = ultra animated** — [ADR-018](./ADR-018-frontend-ui-motion-stack.md) · [FRONTEND_COMPONENTS.md](./FRONTEND_COMPONENTS.md).
...
2. **Epic 1.5** ← **current** — DB + Identity + login + add user (DEV-150…162)
...
| Minimal Identity without roles table | Use full Identity |
...
1. Check [ARCHITECTURE.md](./ARCHITECTURE.md) section 2.1 (domains) and section 9 (auth).
